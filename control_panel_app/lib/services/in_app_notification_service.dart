...
class InAppNotificationService {
  ...
  static Future<void> showNotificationDialog({
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (context != null) {
        debugPrint('[InAppNotificationService] Showing dialog (attempt ${i + 1}) with title="$title"');
        _isShowing = true;
        try {
          ...
          debugPrint('[InAppNotificationService] Dialog closed');
          _isShowing = false;
        }
        return;
      }
      debugPrint('[InAppNotificationService] rootNavigatorKey context is null (attempt ${i + 1}), retrying...');
      await Future.delayed(const Duration(milliseconds: 200));
    }
    debugPrint('[InAppNotificationService] Failed to obtain context for dialog after retries');
  }
